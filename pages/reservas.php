...
<section id="reservasPage">
  ...
  <table class="tabelaHospedes">
    ...
    for ($i = 0; $i < sizeof($reservas); $i++) :
      ...
      $data_entrada = $reservaActual['data_entrada'];
      $dadosEntrada = explode('-', $data_entrada);
      $anoEntrada = $dadosEntrada[0];
      $mesEntrada = $dadosEntrada[1];
      $diaEntrada = $dadosEntrada[2];
      $dataEntrada = "$diaEntrada/$mesEntrada/$anoEntrada";

      $data_saida = $reservaActual['data_saida'];
      $dadosSaida = explode('-', $data_saida);
      $anoSaida = $dadosSaida[0];
      $mesSaida = $dadosSaida[1];
      $diaSaida = $dadosSaida[2];
      $dataSaida = "$diaSaida/$mesSaida/$anoSaida";
    ?>
      <tr>
        <td><?php echo $reservaActual['codigo'] ?></td>
        <td class="hidden940"><?php echo $dataEntrada ?></td>
        <td class="hidden940"><?php echo $dataSaida ?></td>
